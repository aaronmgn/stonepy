...
!!! tip "Verify the install"
    ...
    ```
    This prints the installed version (for example `0.2.5`).
...
!!! warning "Pre-1.0 / alpha"
    `stonepy` is currently pre-1.0 (PyPI Development Status: **3 - Alpha**). The public API may change between releases. Pin a version (for example `stonepy==0.2.5`) if you need reproducible builds, and review the changelog before upgrading.
