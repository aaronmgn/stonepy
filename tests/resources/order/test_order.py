...
@respx.mock
def test_order_returns_response() -> None:
    ...
    try:
        ...
        request = NewStopLimitOrderRequestDTO.model_construct(order_id=99, market_id=123)
        resp = client.order.order(request)
        assert isinstance(resp, ApiTradeOrderResponseDTO)
        assert route.called
        assert route.calls[0].request.method == "POST"
        assert route.calls[0].request.url.path == "/order/newstoplimitorder"
        assert route.calls[0].request.content == b'{"OrderId":99,"MarketId":123}'
    finally:
        client.close()


@respx.mock
def test_order_async() -> None:
    async def run() -> None:
        route = respx.post("https://api.example/order/newstoplimitorder").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            resp = await client.order.order(request)
            assert isinstance(resp, ApiTradeOrderResponseDTO)
            assert route.called
            assert route.calls[0].request.method == "POST"
        finally:
