...
def test_simulate_cancel_order_returns_response() -> None:
    route = respx.post("https://api.example/order/simulate/cancel").mock(
    ...
    try:
        ...
        assert route.calls[0].request.url.path == "/order/simulate/cancel"
    finally:
        client.close()


@respx.mock
def test_simulate_cancel_order_async() -> None:
    async def run() -> None:
        route = respx.post("https://api.example/order/simulate/cancel").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            request = CancelOrderRequestDTO.model_construct()
            resp = await client.order.simulate_cancel_order(request)
            assert isinstance(resp, ApiSimulateTradeOrderResponseDTO)
            assert route.called
            assert route.calls[0].request.method == "POST"
        finally:
