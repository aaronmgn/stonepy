...
def test_simulate_update_trade_returns_response() -> None:
    route = respx.post("https://api.example/order/simulate/updatetradeorder").mock(
    ...
    try:
        ...
        assert route.calls[0].request.url.path == "/order/simulate/updatetradeorder"
    finally:
        client.close()


@respx.mock
def test_simulate_update_trade_async() -> None:
    async def run() -> None:
        route = respx.post("https://api.example/order/simulate/updatetradeorder").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            request = UpdateTradeOrderRequestDTO.model_construct()
            resp = await client.order.simulate_update_trade(request)
            assert isinstance(resp, ApiSimulateTradeOrderResponseDTO)
            assert route.called
            assert route.calls[0].request.method == "POST"
        finally:
