...
def test_update_order_returns_response() -> None:
    route = respx.post("https://api.example/order/updatestoplimitorder").mock(
    ...
    try:
        ...
        assert route.calls[0].request.url.path == "/order/updatestoplimitorder"
    finally:
        client.close()


@respx.mock
def test_update_order_async() -> None:
    async def run() -> None:
        route = respx.post("https://api.example/order/updatestoplimitorder").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            request = UpdateStopLimitOrderRequestDTO.model_construct()
            resp = await client.order.update_order(request)
            assert isinstance(resp, ApiTradeOrderResponseDTO)
            assert route.called
            assert route.calls[0].request.method == "POST"
        finally:
