...
def test_get_markets_trades_wall_returns_response() -> None:
    route = respx.get("https://api.example/order/getmarketstradeswallinfo").mock(
    ...
    try:
        ...
        assert isinstance(resp, GetMarketsTradesWallResponseDTO)
        assert route.called
        assert route.calls[0].request.method == "GET"
        assert route.calls[0].request.url.path == "/order/getmarketstradeswallinfo"
    finally:
        client.close()


@respx.mock
def test_get_markets_trades_wall_async() -> None:
    async def run() -> None:
        route = respx.get("https://api.example/order/getmarketstradeswallinfo").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            market_i_ds = [1]
            number_of_results = 1
            resp = await client.order.get_markets_trades_wall(market_i_ds, number_of_results)
            assert isinstance(resp, GetMarketsTradesWallResponseDTO)
            assert route.called
            assert route.calls[0].request.method == "GET"
        finally:
