...
def test_simulate_order_returns_response() -> None:
    route = respx.post("https://api.example/order/simulate/newstoplimitorder").mock(
    ...
    try:
        ...
        assert route.calls[0].request.url.path == "/order/simulate/newstoplimitorder"
    finally:
        client.close()


@respx.mock
def test_simulate_order_async() -> None:
    async def run() -> None:
        route = respx.post("https://api.example/order/simulate/newstoplimitorder").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            request = NewStopLimitOrderRequestDTO.model_construct()
            resp = await client.order.simulate_order(request)
            assert isinstance(resp, ApiSimulateTradeOrderResponseDTO)
            assert route.called
            assert route.calls[0].request.method == "POST"
        finally:
