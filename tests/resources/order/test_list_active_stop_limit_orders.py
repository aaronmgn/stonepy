...
def test_list_active_stop_limit_orders_returns_response() -> None:
    ...
    try:
        ...
        resp = client.order.list_active_stop_limit_orders(trading_account_id=trading_account_id)
        assert isinstance(resp, ListActiveStopLimitOrderResponseDTO)
        assert route.called
        assert route.calls[0].request.method == "GET"
        assert route.calls[0].request.url.path == "/order/activestoplimitorders"
    finally:
        client.close()


@respx.mock
def test_list_active_stop_limit_orders_async() -> None:
    async def run() -> None:
        route = respx.get("https://api.example/order/activestoplimitorders").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            trading_account_id = 1
            resp = await client.order.list_active_stop_limit_orders(
