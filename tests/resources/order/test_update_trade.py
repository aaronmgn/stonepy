...
@respx.mock
def test_update_trade_returns_response() -> None:
    route = respx.post("https://api.example/order/updatetradeorder").mock(
    ...
    try:
        ...
        resp = client.order.update_trade(request)
        assert isinstance(resp, ApiTradeOrderResponseDTO)
        assert route.called
        assert route.calls[0].request.method == "POST"
        assert route.calls[0].request.url.path == "/order/updatetradeorder"
    finally:
        client.close()


@respx.mock
def test_update_trade_async() -> None:
    async def run() -> None:
        route = respx.post("https://api.example/order/updatetradeorder").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            request = UpdateTradeOrderRequestDTO.model_construct()
            resp = await client.order.update_trade(request)
            assert isinstance(resp, ApiTradeOrderResponseDTO)
            assert route.called
            assert route.calls[0].request.method == "POST"
        finally:
