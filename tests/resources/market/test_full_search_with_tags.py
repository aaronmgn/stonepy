...
@respx.mock
def test_full_search_with_tags_returns_response() -> None:
    route = respx.get("https://api.example/market/fullsearchwithtags").mock(
    ...
    try:
        ...
        resp = client.market.full_search_with_tags(
            query,
        ...
        assert route.calls[0].request.url.path == "/market/fullsearchwithtags"
    finally:
        client.close()


@respx.mock
def test_full_search_with_tags_async() -> None:
    async def run() -> None:
        route = respx.get("https://api.example/market/fullsearchwithtags").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            max_results = 1
            use_mobile_short_name = False
            resp = await client.market.full_search_with_tags(
                query,
                search_by_market_code,
                search_by_market_name,
                spread_product_type,
