...
@respx.mock
def test_list_market_information_search_returns_response() -> None:
    route = respx.get("https://api.example/market/informationsearch").mock(
    ...
    try:
        ...
        resp = client.market.list_market_information_search(
        ...
        assert isinstance(resp, ListMarketInformationSearchResponseDTO)
        assert route.called
        assert route.calls[0].request.method == "GET"
        assert route.calls[0].request.url.path == "/market/informationsearch"
    finally:
        client.close()


@respx.mock
def test_list_market_information_search_async() -> None:
    async def run() -> None:
        route = respx.get("https://api.example/market/informationsearch").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            client_account_id = 1
            use_mobile_short_name = False
            resp = await client.market.list_market_information_search(
                search_by_market_code,
                search_by_market_name,
                spread_product_type,
                cfd_product_type,
