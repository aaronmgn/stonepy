...
def test_trade_fm_returns_response() -> None:
    route = respx.post("https://api.example/fixedmargin/newtradeorder").mock(
    ...
    try:
        ...
        resp = client.fixed_margin.trade_fm(request)
        assert isinstance(resp, FixedMarginOrderResponseDTO)
        assert route.called
        assert route.calls[0].request.method == "POST"
        assert route.calls[0].request.url.path == "/fixedmargin/newtradeorder"
    finally:
        client.close()


@respx.mock
def test_trade_fm_async() -> None:
    async def run() -> None:
        route = respx.post("https://api.example/fixedmargin/newtradeorder").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            request = NewFixedMarginTradeOrderRequestDTO.model_construct()
            resp = await client.fixed_margin.trade_fm(request)
            assert isinstance(resp, FixedMarginOrderResponseDTO)
            assert route.called
            assert route.calls[0].request.method == "POST"
        finally:
