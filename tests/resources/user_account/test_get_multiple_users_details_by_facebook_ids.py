...
def test_get_multiple_users_details_by_facebook_ids_returns_response() -> None:
    route = respx.get("https://api.example/useraccount/getusersbyfacebookids").mock(
    ...
    try:
        ...
        assert resp.ci_connect_users_details[0].facebook_id == "x"
        assert route.called
        assert route.calls[0].request.method == "GET"
        assert route.calls[0].request.url.path == "/useraccount/getusersbyfacebookids"
    finally:
        client.close()


@respx.mock
def test_get_multiple_users_details_by_facebook_ids_async() -> None:
    async def run() -> None:
        route = respx.get("https://api.example/useraccount/getusersbyfacebookids").mock(
            return_value=httpx.Response(200, content=_RESPONSE_BODY)
        )
        ...
        try:
            ...
            resp = await client.user_account.get_multiple_users_details_by_facebook_ids()
            assert isinstance(resp, ApiGetMultipleUsersDetailsResponseDTO)
            assert resp.ci_connect_users_details is not None
            assert resp.ci_connect_users_details[0].facebook_id == "x"
            assert route.called
