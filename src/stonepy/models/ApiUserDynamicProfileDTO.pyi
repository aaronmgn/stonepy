# GENERATED by stonepy._generator. Do not edit by hand.
"""
Represents a CI Connect user's dynamic (calculated details) profile. Note: for use of the CI
Connect social trading platform.
"""

from __future__ import annotations

from typing import overload

from pydantic import Field

from stonepy._core.models import ResponseModel

class ApiUserDynamicProfileDTO(ResponseModel):
    """
    Represents a CI Connect user's dynamic (calculated details) profile. Note: for use of the
    CI Connect social trading platform.
    """

    average_trade_length: int | None = Field(default=None, alias="AverageTradeLength")
    """The user's current average trade length (the period a position is held open) in seconds."""
    last_trade_date_time_utc: str | None = Field(default=None, alias="LastTradeDateTimeUTC")
    """Date and time in UTC of the last trade made by this user."""
    last_trade_direction: str | None = Field(default=None, alias="LastTradeDirection")
    """Direction of the last trade."""
    last_traded_market_id: int | None = Field(default=None, alias="LastTradedMarketId")
    """The identifier of the market last traded by this user."""
    last_traded_market_name: str | None = Field(default=None, alias="LastTradedMarketName")
    """Name of the last market traded by this user."""
    last_traded_underlying_type: str | None = Field(default=None, alias="LastTradedUnderlyingType")
    """The type of the underlying for the market last traded by the user."""
    number_followed: int | None = Field(default=None, alias="NumberFollowed")
    """Number of other CI Connect users followed by this user."""
    number_following: int | None = Field(default=None, alias="NumberFollowing")
    """Number of CI Connect users following this user."""

    @overload
    def __init__(
        self,
        *,
        AverageTradeLength: int | None = ...,
        LastTradeDateTimeUTC: str | None = ...,
        LastTradeDirection: str | None = ...,
        LastTradedMarketId: int | None = ...,
        LastTradedMarketName: str | None = ...,
        LastTradedUnderlyingType: str | None = ...,
        NumberFollowed: int | None = ...,
        NumberFollowing: int | None = ...,
    ) -> None: ...
    @overload
    def __init__(
        self,
        *,
        average_trade_length: int | None = ...,
        last_trade_date_time_utc: str | None = ...,
        last_trade_direction: str | None = ...,
        last_traded_market_id: int | None = ...,
        last_traded_market_name: str | None = ...,
        last_traded_underlying_type: str | None = ...,
        number_followed: int | None = ...,
        number_following: int | None = ...,
    ) -> None: ...
